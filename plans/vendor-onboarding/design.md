...
- Status: Preview ready (revision 5 implemented, account audience added). Design revision 5 is approved. The vendor area with its own eight-item menu, the reference masters, the fixed document slots, the BIM questionnaire and the per-aspect review are implemented, migrated, seeded and checked on the development target. Rendered browser behavior is not verified.
- Account audience (2026-09-17, user report): the admin account showed "My registration" because the entry had no audience rule. The identity payload now reports the owned vendor (`GET /me` returns `vendor: { id, statusCode } | null`), `apps/web/src/framework/audience.ts` derives `vendor` or `staff` from it, the navigation contract gained an `audience` field, and `visibleNavigation` filters by it. `My registration` is `audience: vendor` and `Vendor review` is `audience: staff`, so an HK account sees only the review entry and a vendor account sees only its own menu. The account menu also shows the account kind. Check: `vendors.routes.spec.ts` asserts the identity payload for an owner and for a staff session; `manifest.spec.ts` asserts the two audiences stay apart.
